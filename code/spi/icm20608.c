/*
 * @Author: cpu_code
 * @Date: 2020-06-06 21:01:45
 * @LastEditTime: 2020-06-07 12:26:45
 * @FilePath: \Linux_driver\code\spi\icm20608.c
...
/**
 * @function: 读取icm20608指定寄存器值，读取一个寄存器
 * @parameter: 
 *      dev:  icm20608设备
 *      reg:  要读取的寄存器
 * @return: 
...
/**
 * @function: 向icm20608指定寄存器写入指定的值，写一个寄存器
 * @parameter: 
 *      dev:  icm20608设备
 *      reg:  要写的寄存器
 *      data: 要写入的值
...
/**
 * @function: 读取ICM20608的数据，读取原始数据，包括三轴陀螺仪、三轴加速度计和内部温度。
 * @parameter: 
 *      dev	: ICM20608设备
 * @return: 
 *     success: 
 ...
 * @parameter: 
 *      inode 	: 传递给驱动的inode
 *      filp 	: 设备文件，file结构体有个叫做pr似有ate_data的成员变量
...
/**
 * @function: spi驱动的probe函数，当驱动与设备匹配以后此函数就会执行
 * @parameter: 
 *      client  : spi设备
 *      id      : spi设备ID
 * @return: 
...
/**
 * @function: spi驱动的remove函数，移除spi驱动的时候此函数会执行
 * @parameter: 
 *      client 	: spi设备
 * @return: 
 *     success: 0
