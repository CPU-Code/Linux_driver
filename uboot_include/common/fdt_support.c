/*
 * @Author: cpu_code
 * @Date: 2020-05-19 19:40:35
 * @LastEditTime: 2020-05-19 19:50:09
 * @FilePath: \Linux_driver\uboot_include\common\fdt_support.c
 ...
 * boot_fn() boot_fn = do_bootm_linux函数, 实际运行函数do_bootm_linux() -> 
 * boot_prep_linux()启动Linux之前做一些其他处理，
 * 		比如在设备树的chosen节点下添加子节点bootargs，bootargs子节点存放bootargs环境变量 ->
 * image_setup_linux() -> image_setup_libfdt() -> 
 * fdt_chosen() 最终调用fdt_chosen函数在chosen节点中添加bootargs属性
 */
int fdt_chosen(void *fdt)
{
	...
	/* find or create "/chosen" node. */
	/* 从设备树(.dtb)中找到 chosen 节点，如 没有找到的话就会自己创建一个 chosen 节点 */
	nodeoffset = fdt_find_or_add_subnode(fdt, 0, "chosen");
	if (nodeoffset < 0)
		return nodeoffset;
	...
	if(str) 
	{
		/* 向 chosen 节点添加 bootargs 属性，且 bootargs 属性的值就是环境变量 bootargs 的内容 */
